...
class Service(models.Model):
    ...
    def __str__(self):
        ...
        Returns:
            str: Service name, vendor, and type in readable format
        """
        vendor_name = getattr(self.vendor, "username", "Unknown Vendor")
        service_type_display = dict(self.SERVICE_TYPE_CHOICES).get(
            self.service_type, self.service_type
        )
        return f"{self.name} by {vendor_name} ({service_type_display})"
